...
public interface StudentRepository extends JpaRepository<Student, Long> {
        ...
        @Query(value = "SELECT s.* FROM student s INNER JOIN student_course sc ON s.student_id = sc.student_id " +
                        ...
                        "AND (?4 IS NULL OR c.course_name = ?4)", countQuery = "SELECT COUNT(*) FROM student s INNER JOIN student_course sc ON s.student_id = sc.student_id "
                                        +
                                        "INNER JOIN course c ON sc.course_id = c.course_id WHERE " +
                                        "s.campus_name = ?1 " +
                                        "AND (?2 IS NULL OR s.grade = ?2) " +
        ...
        Page<Student> findByGradeAndSectionNameAndStudentCourses(String campusName, String grade, String sectionName,
                        String course, Pageable page);

        @Query(value = "SELECT * FROM student WHERE campus_name=?1", countQuery = "SELECT COUNT(*) FROM student WHERE campus_name=?1", nativeQuery = true)
        Page<Student> findbycampus(String campusName, Pageable pageable);

        @Query(value = "SELECT s.* FROM student s INNER JOIN student_course sc ON s.student_id = sc.student_id " +
                        "INNER JOIN course c ON sc.course_id = c.course_id WHERE " +
                        "s.campus_name = ?1 and c.course_name=?2", countQuery = "SELECT COUNT(*) FROM student s INNER JOIN student_course sc ON s.student_id = sc.student_id "
                                        +
                                        "INNER JOIN course c ON sc.course_id = c.course_id WHERE " +
                                        "s.campus_name = ?1 and c.course_name=?2", nativeQuery = true)
        Page<Student> findbycampusAndCourse(String campusName, String course, Pageable page);

        @Query(value = "SELECT s.* FROM student s INNER JOIN student_course sc ON s.student_id = sc.student_id " +
                        "INNER JOIN course c ON sc.course_id = c.course_id WHERE " +
                        "s.campus_name = ?1 and c.course_name=?2 and s.grade=?3", countQuery = "SELECT COUNT(*) FROM student s INNER JOIN student_course sc ON s.student_id = sc.student_id "
                                        +
                                        "INNER JOIN course c ON sc.course_id = c.course_id WHERE " +
                                        "s.campus_name = ?1 and c.course_name=?2 and s.grade=?3", nativeQuery = true)
        Page<Student> findbycampusAndCourseAndGrade(String campusName, String course, String grade, Pageable page);

        @Query(value = "SELECT s.* FROM student s INNER JOIN student_course sc ON s.student_id = sc.student_id " +
                        "INNER JOIN course c ON sc.course_id = c.course_id WHERE " +
                        "s.campus_name = ?1 and c.course_name=?2 and s.grade=?3 and s.section_name=?4", countQuery = "SELECT COUNT(s.*) FROM student s INNER JOIN student_course sc ON s.student_id = sc.student_id "
                                        +
                                        "INNER JOIN course c ON sc.course_id = c.course_id WHERE " +
                                        "s.campus_name = ?1 and c.course_name=?2 and s.grade=?3 and s.section_name=?4", nativeQuery = true)
        Page<Student> findbycampusAndCourseAndGradeAndSection(String campusName, String course, String grade,
                        String section, Pageable page);

        @Query(value = "SELECT s.* FROM student s WHERE " +
                        "s.campus_name = ?1 and s.grade=?2 and s.section_name=?3", countQuery = "SELECT COUNT(*) FROM student s WHERE "
                                        +
                                        "s.campus_name = ?1 and s.grade=?2 and s.section_name=?3", nativeQuery = true)
        Page<Student> findbycampusAndGradeAndSection(String campusName, String grade, String section, Pageable page);

        @Query(value = "SELECT s.* FROM student s WHERE " +
                        "s.campus_name = ?1 and s.section_name=?2", countQuery = "SELECT COUNT(*) FROM student s WHERE "
                                        +
                                        "s.campus_name = ?1 and s.section_name=?2", nativeQuery = true)
        Page<Student> findbycampusAndSection(String campusName, String section, Pageable page);

        @Query(value = "SELECT s.* FROM student s WHERE " +
                        "s.campus_name = ?1 and s.grade=?2", countQuery = "SELECT COUNT(*) FROM student s WHERE " +
                                        "s.campus_name = ?1 and s.grade=?2", nativeQuery = true)
        ...
        @Query(value = "SELECT s FROM Student s WHERE s.campusName = :campusName", countQuery = "SELECT count(s) FROM Student s WHERE s.campusName = :campusName")
        ...
        List<Student> SearchByRollNumber(@Param("rollNumber") String rollNumber);
}
