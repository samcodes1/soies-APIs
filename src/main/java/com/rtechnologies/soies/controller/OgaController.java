...
public class OgaController {
    ...
    public ResponseEntity<OgaDtoResposne> getOgasByCourseId(@PathVariable Long courseId,
                                                             @PathVariable String studentRollNumber) {
        OgaDtoResposne response = ogaService.getOgasByCourseId(courseId, studentRollNumber);
    ...
    @ApiOperation(value = "Get submitted OGAs by course ID with pagination", response = OgaSubmissionsResponse.class)
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "OGAs retrieved successfully"),
            @ApiResponse(code = 404, message = "No OGAs found for the given course ID"),
            @ApiResponse(code = 500, message = "Internal server error")
    ...
    public ResponseEntity<OgaSubmissionsResponse> getOgaSubmissionsByCourseId(
            @PathVariable Long courseId,
            @RequestParam(defaultValue = "0") int page, // Default page number
            @RequestParam(defaultValue = "10") int size // Default page size
    ) {
        OgaSubmissionsResponse response = ogaService.getOgaSubmissionsByCourseId(courseId, page, size);
        return ResponseEntity.ok(response);
    }
}
