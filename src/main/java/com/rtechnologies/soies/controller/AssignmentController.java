...
public class AssignmentController {
        ...
        public ResponseEntity<AssignmentSubmissionResponse> getAssignmentSubmissions(
                ...
                AssignmentSubmissionResponse studentListResponse = assignmentService
                                .getAssignmentSubmissionById(assignmentId, studentRollNumber);
        ...
        public ResponseEntity<AssignmentSubmissionResponse> markAssignment(
                        @RequestBody MarkAssignmentRequest assignment) {
        ...
        @ApiOperation(value = "Get assignments by course ID and Grade", response = AssignmentListResponse.class)
        @ApiResponses(value = {
                        @ApiResponse(code = 200, message = "Assignments retrieved successfully"),
                        @ApiResponse(code = 404, message = "No assignments found for the given course ID and Grade"),
                        @ApiResponse(code = 500, message = "Internal server error")
        })
        @GetMapping("/getByCourseAndGrade/{courseId}/{grade}")
        public ResponseEntity<AssignmentListResponse> getAssignmentsByCourseIdAndGrade(@PathVariable Long courseId,
                        @PathVariable String grade) {
                AssignmentListResponse response = assignmentService.getAssignmentsByCourseAndGrade(courseId, grade);
                return ResponseEntity.status(response.getMessageStatus().equals("Success") ? 200 : 500).body(response);
        }
